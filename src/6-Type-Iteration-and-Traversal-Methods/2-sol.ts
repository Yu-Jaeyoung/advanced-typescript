...
type ExtractResults<Ops extends unknown[]> =
  Ops extends [ Promise<infer F>, ...infer Rest ]
    ? [ F, ...ExtractResults<Rest> ]
    : [];

/**
 * Wraps the extracted tuple in a Promise.
 */
type ResolvedResults<Ops extends unknown[]> = Promise<ExtractResults<Ops>>;
